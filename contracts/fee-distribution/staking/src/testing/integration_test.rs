...
fn test_stake_unstake_claim() {
    ...
    // update tokens per interval
    {
        ...
        let config: Config = wasm.query(&staking_address, &QueryMsg::Config {}).unwrap();
        assert_eq!(config.tokens_per_interval, Uint128::from(new_tokens_per_interval));
    }
    ...
    // stake then increase time by one day
    {
        ...
        // check claimable
        {
            let claimable: Uint128 = wasm
                ...
                .unwrap();
            assert_eq!(claimable, Uint128::from(1_785_715_000u128 + 1_190_579_000u128));
        ...
        {
            assert_eq!(
                env.get_balance(env.traders[0].address(), config.staked_denom),
                Uint128::zero()
            );
        ...
        // check claimable
        {
            ...
            let claimable: Uint128 = wasm
                ...
                .unwrap();
            assert_eq!(claimable, Uint128::from(595_238_000u128 + 1_786_025_000u128));
        }
    }
}
