...
use osmosis_std::{
    ...
    types::osmosis::{
        gamm::v1beta1::QueryTotalPoolLiquidityRequest,
...
pub fn validate_pool_route(
    ...
    for route_part in &pool_route {
        ...
        // return error.
        if !liquidity.iter().any(|coin| coin.denom == route_part.token_out_denom) {
            return Result::Err(ContractError::InvalidPoolRoute {
                reason: format!(
                    "denom {} is not in pool id {}",
                    current_denom_in, route_part.pool_id
...
pub fn calculate_min_output_from_twap(
    ...
    deps.api.debug(&format!("twap_price minus {percentage_impact}%: {twap_price}"));
